    ...
    uint64_t total_height)
{
    ...
    for (source_index = 0U; source_index < document->source_count; ++source_index) {
        ...
        for (hint_index = 0U; hint_index < source->hint_count; ++hint_index) {
            ...
            /* Automatic hints only steer the break near the ideal page height.
               A manual hint expresses explicit user intent, so it stays
               actionable across the whole legal page-height range; otherwise a
               dragged seam would snap back once it left the search radius. */
            uint64_t window_minimum = hint->kind == LOIM_SPLIT_HINT_MANUAL
                ? legal_minimum
                : search_minimum;
            uint64_t window_maximum = hint->kind == LOIM_SPLIT_HINT_MANUAL
                ? legal_maximum
                : search_maximum;

            if (candidate >= window_minimum && candidate <= window_maximum) {
                loim_consider_break(
                    candidate,
